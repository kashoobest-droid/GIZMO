...
<body>
    <div class="email-container">
        ...
        <div class="content">
            <div class="title">Payment Rejected — Order #{{ $order->id }}</div>
            <p class="muted">Hi {{ $order->user->name }},</p>
            <p>We're sorry — the payment you submitted for <strong>Order #{{ $order->id }}</strong> could not be verified.</p>
            <p>Order total: <strong>@currency($order->total)</strong></p>
            @if(!empty($reason))
                <div style="background:#fff3f3;padding:12px;border-left:4px solid #DC143C;border-radius:6px;margin:12px 0;">
                    <strong style="color:#DC143C;display:block;margin-bottom:6px;">Reason for rejection</strong>
                    <div style="color:#333;">{{ $reason }}</div>
                </div>
            @endif
            <p>Please contact our support team or retry the payment with a valid transaction screenshot/receipt.</p>

            @if(!empty($editUrl))
                <p style="margin-top:12px;">
                    <a href="{{ $editUrl }}" class="cta-button">Update payment &amp; re-upload receipt</a>
                </p>
            @else
                <p>
            ...
            <p class="muted">If you believe this is a mistake, reply to this email and our support team will review it.</p>
        </div>
